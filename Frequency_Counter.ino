...
volatile uint16_t prescaler=64;
...
uint8_t printSetPrescaler() {
  ...
  if (command.startsWith("Prescaler ")) {
    prescaler = command.substring(10).toInt();
    Serial.print("Value = "); Serial.println(prescaler);
    prescaler = SelectPrescaler(prescaler); // Este valor se corresponde con el que los bits CS2x del timer 2 requiere
    if (prescaler != 8) {
       Serial.print("Value = "); Serial.println(prescaler);
       return; 
    }else {
